...
export default async function HomePage() {
  try {
    ...
    const active = companies.filter((company) =>
      company.recruitments?.some((program) =>
      ...
      <>
        <section className="terminal-hero">
          <div className="shell terminal-hero-grid">
            <div className="hero-copy">
              ...
              <h1>车辆行业校招情报</h1>
              <p className="hero-lead">官方入口、招聘批次与核验证据。先判断链接是什么、是否有效、何时核验，再决定下一步。</p>
              <div className="hero-actions">
                <Link className="button button-primary" href="#company-search">开始搜索<ArrowRight size={16} /></Link>
                <Link className="button button-secondary" href="/about">了解核验规则</Link>
              </div>
            </div>
            <aside className="signal-panel" aria-label="平台数据快照">
              <span className="signal-line"><i />LINK EVIDENCE · 2026-06-11</span>
              <div className="signal-metrics">
                <p><strong>{companies.length}</strong><span>跟踪企业</span></p>
                <p><strong>{active.length}</strong><span>明确 2027 机会</span></p>
              </div>
        ...
        <section className="shell page-section search-first" id="company-search">
          <div className="section-heading">
            <div><p className="eyebrow">SEARCH FIRST</p><h2>先找公司，再看证据</h2></div>
        ...
        <section className="shell page-section">
          <div className="section-heading">
            <div><p className="eyebrow">OPEN NOW</p><h2>当前明确开放的 2027 项目</h2></div>
            ...
            {active.map((company) => {
              ...
              return (
                <article className="opportunity-row" key={company.id}>
                  <div><span>{company.type}</span><h3><Link href={`/companies/${company.slug}`}>{company.name}</Link></h3></div>
                  <p>{program?.title}<small>核验于 {date(program?.verifiedAt)} · 截止日期未公布</small></p>
        ...
        <section className="shell page-section intelligence-grid">
          <div>
            <div className="section-heading compact-heading"><div><p className="eyebrow">RECENT CHANGES</p><h2>最近核验变化</h2></div></div>
          ...
          <aside className="method-panel">
            ...
            <h2>链接先解释，按钮后出现</h2>
            <ol>
              <li><SearchCheck size={15} />先看来源类型、官方域名和面向届次。</li>
              <li><Clock3 size={15} />再看健康状态与最后核验时间。</li>
              <li><ShieldCheck size={15} />失效或受限链接不会伪装成正常主按钮。</li>
            </ol>
            <Link href="/about">查看完整证据规则<ArrowRight size={15} /></Link>
          </aside>
        </section>
      </>
    );
