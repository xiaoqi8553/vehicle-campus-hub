...
export default async function ResourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="shell page-space resource-article-page">
      <Link className="back-link" href="/resources"><ArrowLeft size={16} />返回资料库</Link>
      ...
      <div className="resource-article-layout">
        <nav aria-label="文章目录" className="article-toc">
          ...
          {resource.content.map((section, index) => (
            <a href={`#section-${index + 1}`} key={section.heading}>{String(index + 1).padStart(2, "0")} {section.heading}</a>
          ))}
        </nav>
        <article className="resource-article">
          {resource.content.map((section, index) => (
            <section id={`section-${index + 1}`} key={section.heading}>
              ...
              <ul>{section.checklist.map((item) => <li key={item}><CheckCircle2 size={17} />{item}</li>)}</ul>
            </section>
          ))}
        </article>
      </div>
