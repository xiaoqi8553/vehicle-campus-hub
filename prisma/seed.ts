  ...
  for (const [index, item] of companies.entries()) {
    ...
    await prisma.resource.createMany({
      data: [
        {
          companyId: company.id,
          title: `${item.name} ${interviewTitles[index % interviewTitles.length]}`,
          type: "面经",
          url: null,
          source: "候选人经验汇总（待核实）",
        ...
        {
          companyId: company.id,
          title: `${item.name} ${guideTitles[index % guideTitles.length]}`,
          type: "投递攻略",
          url: officialResourceUrls[item.id] ?? null,
          source: officialResourceUrls[item.id] ? "企业招聘官网" : "公开信息整理（待补链接）",
          summary: guideSummaries[index % guideSummaries.length],
          credibility: officialResourceUrls[item.id] ? "官方" : "较可信",
        },
      ],
    });
