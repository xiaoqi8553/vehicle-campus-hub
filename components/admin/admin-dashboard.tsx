      ...
      <div className="admin-main">
        ...
        {active === "companies" && (
          <>
            <AdminForm
              title="新增公司"
              onSubmit={(event) => submit(event, "/api/companies")}
              pending={pending}
            >
              <input aria-label="公司名称" name="name" placeholder="公司名称 *" required />
              <select aria-label="公司类型" name="category" required defaultValue="">
              ...
              <select aria-label="公司校招状态" name="status" required defaultValue="待确认">
        ...
        {active === "recruitments" && (
          <>
            <AdminForm
              title="新增校招项目"
              onSubmit={(event) => submit(event, "/api/recruitments")}
              pending={pending}
            >
              ...
              <select aria-label="校招项目状态" name="status" required defaultValue="待确认">
                {RECRUITMENT_STATUSES.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
        ...
        {active === "jobs" && (
          <>
            <AdminForm
              title="新增岗位"
              onSubmit={(event) => submit(event, "/api/jobs")}
              pending={pending}
            >
              <CompanySelect companies={data.companies} />
              <input aria-label="岗位名称" name="title" placeholder="岗位名称 *" required />
              <select aria-label="岗位方向" name="direction" required defaultValue="">
                <option value="" disabled>
                  岗位方向 *
                </option>
        ...
        {active === "resources" && (
          <>
            <AdminForm
              title="新增资料"
              onSubmit={(event) => submit(event, "/api/resources")}
              pending={pending}
            >
              <CompanySelect companies={data.companies} />
              <input aria-label="资料标题" name="title" placeholder="资料标题 *" required />
              <select aria-label="资料类型" name="type" required>
                {RESOURCE_TYPES.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
              <select aria-label="资料可信度" name="credibility" required>
                {CREDIBILITY_LEVELS.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
        ...
        {active === "calendarEvents" && (
          <>
            <AdminForm
              title="新增日历事件"
              onSubmit={(event) => submit(event, "/api/calendar-events")}
              pending={pending}
            >
              <CompanySelect companies={data.companies} />
              <select aria-label="日历事件类型" name="eventType" required>
              ...
              <select aria-label="日历事件状态" name="status" required defaultValue="待确认">
                {RECRUITMENT_STATUSES.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
              <select aria-label="日历事件可信度" name="credibility" required defaultValue="待核实">
                {CREDIBILITY_LEVELS.map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
...
function CompanyEditorRow({
  ...
  return (
    <tr>
      ...
      <td>
        <input
          aria-label={`${company.name} 校招官网`}
          value={campusUrl}
          onChange={(event) => setCampusUrl(event.target.value)}
          placeholder="可空"
        />
      </td>
      <td>
        <div className="table-actions">
          <button
            disabled={pending}
            onClick={() => onUpdate(company.id, { status, officialWebsite, campusUrl })}
          >
            保存
          </button>
          <button className="danger" disabled={pending} onClick={() => onDelete(company.id)}>
            删除
          </button>
        </div>
      </td>
  ...
  return (
    <div className="table-wrap">
      <table>
        ...
        <tbody>
          {items.map((item) => (
            <RecruitmentEditorRow key={item.id} item={item} pending={pending} onUpdate={onUpdate} />
          ))}
        </tbody>
      </table>
    </div>
...
function RecruitmentEditorRow({
  ...
  return (
    <tr>
      ...
      <td>
        <select
          aria-label={`${item.companyName} 项目状态`}
          value={status}
          onChange={(event) => setStatus(event.target.value)}
        >
          {RECRUITMENT_STATUSES.map((value) => (
      ...
      <td>
        <input
          aria-label={`${item.companyName} 开始日期`}
          type="date"
          value={startDate}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 截止日期`}
          type="date"
          value={endDate}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 投递链接`}
          value={applyUrl}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 招聘流程`}
          value={process}
  ...
  return (
    <div className="table-wrap">
      <table>
        ...
        <tbody>
          {items.map((item) => (
            <JobEditorRow key={item.id} item={item} pending={pending} onUpdate={onUpdate} />
          ))}
        </tbody>
      </table>
    </div>
...
function JobEditorRow({
  ...
  return (
    <tr>
      ...
      <td>
        <select
          aria-label={`${item.companyName} 岗位方向`}
          value={direction}
          onChange={(event) => setDirection(event.target.value)}
        >
          {JOB_DIRECTIONS.map((value) => (
            <option key={value}>{value}</option>
          ))}
        </select>
      </td>
      <td>
        <input
          aria-label={`${item.companyName} 岗位城市`}
          value={city}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 学历要求`}
          value={education}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 专业要求`}
          value={majorRequirement}
          onChange={(event) => setMajorRequirement(event.target.value)}
        />
      </td>
      <td>
        <button
          className="table-save"
          disabled={pending}
          onClick={() => onUpdate(item.id, { direction, city, education, majorRequirement })}
        >
          保存
        </button>
      </td>
  ...
  return (
    <div className="table-wrap">
      <table>
        ...
        <tbody>
          {items.map((item) => (
            <ResourceEditorRow key={item.id} item={item} pending={pending} onUpdate={onUpdate} />
          ))}
        </tbody>
      </table>
    </div>
...
function ResourceEditorRow({
  ...
  return (
    <tr>
      ...
      <td>
        <select
          aria-label={`${item.companyName} 资料类型`}
          value={type}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 资料来源`}
          value={source}
      ...
      <td>
        <select
          aria-label={`${item.companyName} 资料可信度`}
          value={credibility}
          onChange={(event) => setCredibility(event.target.value)}
        >
          {CREDIBILITY_LEVELS.map((value) => (
            <option key={value}>{value}</option>
          ))}
        </select>
      </td>
      <td>
        <input
          aria-label={`${item.companyName} 资料链接`}
          value={url}
      ...
      <td>
        <button
          className="table-save"
          disabled={pending}
          onClick={() => onUpdate(item.id, { type, source, credibility, url })}
        >
          保存
        </button>
      </td>
...
function CalendarEventEditorRow({
  item,
  pending,
  onUpdate,
}: {
  ...
  return (
    <tr>
      ...
      <td>
        <select
          aria-label={`${item.companyName} 事件类型`}
          value={eventType}
      ...
      <td>
        <input
          aria-label={`${item.companyName} 事件日期`}
          type="date"
          value={eventDate}
      ...
      <td>
        <select
          aria-label={`${item.companyName} 事件状态`}
          value={status}
          onChange={(event) => setStatus(event.target.value)}
        >
          {RECRUITMENT_STATUSES.map((value) => (
      ...
      <td>
        <select
          aria-label={`${item.companyName} 事件可信度`}
          value={credibility}
          onChange={(event) => setCredibility(event.target.value)}
        >
          {CREDIBILITY_LEVELS.map((value) => (
      ...
      <td>
        <input
          aria-label={`${item.companyName} 事件来源`}
          value={sourceUrl}
