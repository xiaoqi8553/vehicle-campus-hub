...
export function CompanyExplorer({
  ...
  const filtered = useMemo(() => {
    ...
    return companies.filter((company) => {
      ...
      const hasUsableLink = company.links?.some((link) =>
        link.isPrimary && isUsableLinkEvidence(link),
      ) ?? false;
      ...
      const opportunity = (company: CompanyCardData) => Number(Boolean(
        company.recruitments?.some((item) => item.targetYear === 2027 && item.status.includes("开放"))
        && company.links?.some((link) => isCohortEvidence(link, 2027)),
      ));
      const verified = (company: CompanyCardData) => Math.max(
        0,
        ...(company.links ?? []).map((link) => link.verifiedAt ? Date.parse(link.verifiedAt) : 0),
      );
    ...
    <div className="explorer" data-testid="company-explorer">
      <div className="filter-panel">
        <label className="search-field">
          <Search size={18} />
          <input type="search" aria-label="搜索公司、城市或车辆方向" placeholder="搜索公司、城市或车辆方向" value={query} onChange={(event) => setQuery(event.target.value)} />
        </label>
        <div className="filter-grid">
          ...
          <label><span>届次</span><select aria-label="届次" defaultValue="2027"><option value="2027">2027 届</option></select></label>
          ...
          {showSort && <label><span>排序</span><select aria-label="排序方式" value={sort} onChange={(event) => setSort(event.target.value)}><option value="opportunity">明确开放优先</option><option value="updated">最近更新</option><option value="name">公司名称</option></select></label>}
        </div>
        <button aria-expanded={advancedOpen} className="advanced-toggle" type="button" onClick={() => setAdvancedOpen((value) => !value)}>
        ...
        {advancedOpen && (
          <div className="advanced-filter-grid">
            ...
            <label><span>链接状态</span><select aria-label="链接状态" value={hasOfficialLink} onChange={(event) => setHasOfficialLink(event.target.value)}><option value="">全部</option><option value="yes">有可用入口</option><option value="no">需人工复核</option></select></label>
          </div>
        )}
      </div>
