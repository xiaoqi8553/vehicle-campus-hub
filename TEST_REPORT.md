...
## 2026-06-13 车招雷达公众官网重塑

### 原有问题

1. 公众品牌仍使用 `VCH` 和 Vehicle Campus Hub，学生难以快速理解用途。
2. 首页、公司库和详情页大量使用内部审计语言，视觉偏台账。
3. 多个公众页面源码存在乱码文案。
4. 首页与公司库职责重复，移动端信息路径不够清楚。
5. 日历重复铺开 22 家待确认企业。
6. 求职资料仍像库存列表，没有突出完整阅读路径。

### 页面与组件变化

- 新增车招雷达品牌、公众导航、统一页脚和页面 Hero。
- 首页改为搜索、真实统计、3 个明确机会、5 条真实变化、技术方向和核验说明。
- 公司库改为学生可读的机会比较列表。
- 公司详情改为当前机会、方向参考、来源记录、FAQ 和快速行动建议。
- 日历只展示可靠时间线、3 个无截止项目和单一观察名单入口。
- 求职指南展示 1 篇路线图和 5 篇完整主题指南。
- 关于页说明使命、数据原则、不会做什么、反馈和后续计划。

### 四视口结果

| 视口              | 结果                                   |
| ----------------- | -------------------------------------- |
| iPhone SE 375×667 | 通过，无横向滚动，主要操作热区符合要求 |
| iPhone 14 390×844 | 通过，首页只展示 3 个机会              |
| iPad 820×1180     | 通过，筛选、卡片和详情无挤压           |
| Desktop 1440×1000 | 通过，Hero、列表和多栏布局正常         |

视觉截图位于 `docs/redesign/2026-06-13/`。
...
| `npm run test:e2e`     | 66 passed / 2 intentional skipped |
| `npm run lint`         | 通过                              |
| `npm run typecheck`    | 通过                              |
| `npm run check:utf8`   | 通过                              |
